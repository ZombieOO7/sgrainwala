...
if(isset($_POST['email']) && $_POST['email'] != '' && isset($_POST['password']) && $_POST['password'] != '') {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        $sql_query = "SELECT *,(SELECT name FROM area a WHERE a.id=u.area) as area_name,(SELECT name FROM city c WHERE c.id=u.city) as city_name FROM `users` u WHERE `email` = '".$username."' OR `mobile` = '".$username."' AND `password` ='".$password."'";
        $db->sql($sql_query);
        $result=$db->getResult();
		if ($db->numRows($result) > 0) {
			...
			if(!empty($fcm_id)){
				...
				//  $sql_query_wallet = "SELECT sum( amount ) FROM `wallet_transactions` WHERE user_id=".$result[0]['id'];
    ...
    //             }
			...
			$response['error']     = true;
			$response['message']   = "Invalid Email / Password passed!";
			// echo json_encode($response);
		}
    }
    print_r(json_encode($response));
    ...
    // if (empty($username) or ($password)) {
        $response['message'] = "Email and password should be filled";
        // echo json_encode($response);
    // }
