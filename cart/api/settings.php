...
if(isset($_POST['settings']) ) {
    ...
    // }
     ...
     if(isset($_POST['get_aboutus'])){
        $sql = "select value from `settings` where id=14";
        $db->sql($sql);
        $res = $db->getResult();
        if(!empty($res)){
            $settings['error'] = false;
            $settings['about_us'] = $res[0]['value'];
            print_r(json_encode($settings));
    ...
    if(isset($_POST['get_terms'])){
        $sql = "select value from `settings` where id=10";
        $db->sql($sql);
        $res = $db->getResult();
        if(!empty($res)){
            $settings['error'] = false;
    ...
    if(isset($_POST['get_logo'])){
        $sql = "select value from `settings` where id=6";
        $db->sql($sql);
        $res = $db->getResult();
        if(!empty($res)){
            $settings['error'] = false;
    ...
    if(isset($_POST['get_contact'])){
        $sql = "select value from `settings` where id=12";
        $db->sql($sql);
        $res = $db->getResult();
        if(!empty($res)){
            $settings['error'] = false;
