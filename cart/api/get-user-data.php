    ...
    if(isset($_POST['user_id']) && $_POST['user_id'] != '') {
        ...
        if ($db->numRows($result) > 0) {
            ...
            foreach($result as $row) {
                ...
                $sql_query_wallet = "SELECT * FROM `wallet_transactions` WHERE user_id=".$row['id'];
                $db->sql($sql_query_wallet);
                $result_wallet=$db->getResult();
                if ($db->numRows($result_wallet) > 0) {
                    $amount = 0;
                    foreach($result_wallet as $rsw){
                        if(strtolower($rsw['type']) == 'credit'){
                            $amount = $amount + $rsw['amount'];
                        }
                        if(strtolower($rsw['type']) == 'debit'){
                            $amount = $amount - $rsw['amount'];
                        }
                    }
