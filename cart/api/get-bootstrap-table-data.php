...
// 			print_r($items);
		...
		/*foreach($status as $st){
			...
			$status = $temp;
			 $box = '<input type="checkbox" class="checkbox checkBoxClass" name="ids[]" value="'.$row['id'].'"/>';
		   
			$operate1 = "<a class='btn btn-sm btn-primary edit-fees' data-id='".$row['id']."' data-toggle='modal' data-target='#editFeesModal'>Edit</a>";
			
			$operate1 .= "<a onclick='return conf(\"delete\");' class='btn btn-sm btn-danger' href='../public/db_operations.php?id=".$row['id']."&delete_order=1' target='_blank'>Delete</a>";
		    $operate = '<a href="order-detail.php?id='.$row['id'].'"><i class="fa fa-eye"></i> View</a>
				<br><a href="delete-order.php?id='.$row['id'].'"><i class="fa fa-trash"></i> Delete</a>';
			$status_chg = '<select name="st" class="form-control actions show-tick" >
                                     ...
                                     if($row["active_status"]=="received") 
                                     { $received = "selected"; }
                                     else if($row["active_status"]=="processed") 
                                     { $processed = "selected"; }
                                     else if($row["active_status"]=="shipped") 
                                     { $shipped = "selected"; }
                                     else if($row["active_status"]=="delivered") 
                                     { $delivered = "selected"; }
                                     else if($row["active_status"]=="returned") 
                                     { $returned = "selected"; }
                                     else if($row["active_status"]=="cancelled") 
                                     { $cancelled = "selected"; }
            $status_chg .= '         <option value="received" id="'.$row['id'].'" '.$received.'>Received</option>
                                    <option value="processed" id="'.$row['id'].'" '.$processed.'>Processed</option>
                                    <option value="shipped" id="'.$row['id'].'" '.$shipped.'>Shipped</option>
                                    <option value="delivered" id="'.$row['id'].'" '.$delivered.'>Delivered</option>
                                    <option value="cancelled" id="'.$row['id'].'" '.$cancelled.'>Cancel</option>
                                    <option value="returned" id="'.$row['id'].'" '.$returned.'>Returned</option>
                                    
                                    </select>';
            //$tempRow['checkbox_check'] = '<input data="'.$row['id'].'" class="checkbox_order" type="checkbox" name="checkorder_'.$row['id'].'" value="1" />';
			$tempRow['id'] = $row['id'];
...
// 			$tempRow['final_total'] = ceil($total-$discounted_amount);
			...
			$tempRow['active_status'] = '<span id="change_action_'.$row['id'].'">'.$active_status.'</span>';
			$tempRow['wallet_balance'] = $row['wallet_balance'];
			$tempRow['date_added'] = date('d-m-Y',strtotime($row['date_added']));
			$tempRow['operate'] = $operate; 
			$tempRow['operate1'] = $box; 
			$tempRow['action_status'] = $status_chg;
...
// 		$sql = "SELECT * FROM products ".$where." ORDER BY ".$sort." ".$order." LIMIT ".$offset.", ".$limit;
        $sql = "SELECT p.id AS id, p.name, p.image, pv.price, pv.measurement, pv.serve_for, pv.stock, u.short_code 
            FROM `products` p
            $join 
	...
	if(isset($_GET['table']) && $_GET['table'] == 'products123'){
// 		print_r($_GET);
		...
		if(isset($_GET['search']) AND $_GET['search']!=''){
			$search = $_GET['search'];
			$where = " where (p.`id` like '%".$search."%' OR p.`name` like '%".$search."%' OR pv.`measurement` like '%".$search."%' OR u.`short_code` like '%".$search."%') GROUP BY p.id";
	...
	//	$join = "JOIN `product_variant` pv ON pv.product_id = p.id
         //   LEFT JOIN `unit` u ON u.id = pv.measurement_unit_id GROUP BY p.id";
...
// 		$sql = "SELECT * FROM products ".$where." ORDER BY ".$sort." ".$order." LIMIT ".$offset.", ".$limit;
	...
	if(isset($_GET['table']) && $_GET['table'] == 'users'){
		...
		if(isset($_GET['search'])){
			$search = $_GET['search'];
			$where = " Where `id` like '%".$search."%' OR `name` like '%".$search."%' OR `email` like '%".$search."%' OR `mobile` like '%".$search."%' OR `city` 
			like '%".$search."%' OR `area` like '%".$search."%' OR `street` like '%".$search."%' OR `status` like '%".$search."%' OR `created_at` like '%".$search."%'";
		}
		...
		foreach($res as $row){  
		    $operate = ' <a class="btn-xs btn-danger" onclick="return confirm("Are you sure?")" href="delete_customers.php?id='.$row['id'].'"><i class="fa fa-trash-o"></i>Delete</a>';
		    $operate_edit = ' <a class="btn-xs btn-info" href="edit_customers.php?id='.$row['id'].'"><i class="fa fa-edit"></i>Edit</a>';
		    $tempRow['id'] = $row['id'];
			$tempRow['name'] = $row['name'];
			...
			$tempRow['created_at'] = $row['created_at'];
			 $tempRow['operate'] = $operate;
			 $tempRow['operate_edit'] = $operate_edit;
			$rows[] = $tempRow;
		}
		$bulkData['rows'] = $rows;
		print_r(json_encode($bulkData));
...
// data of 'Promo Codes' table goes here
	...
	// data of 'Wallet Codes' table goes here
	if(isset($_GET['table']) && $_GET['table'] == 'wallet'){
		...
		if(isset($_GET['search']) && $_GET['search'] !=''){
			$search = $_GET['search'];
			$where = " Where `id` like '%".$search."%' OR `wallet_transactions` like '%".$search."%' OR `message` like '%".$search."%' OR `date` like '%".$search."%' ";
		}
		
		$sql = "SELECT COUNT(id) as total FROM `wallet_transactions`".$where;
		$db->sql($sql);
		$res = $db->getResult();
		foreach($res as $row)
			$total = $row['total'];
		
		$sql = "SELECT * FROM `wallet_transactions`".$where." ORDER BY ".$sort." ".$order." LIMIT ".$offset.", ".$limit;
		//echo $sql;
		$db->sql($sql);
		$res = $db->getResult();
		...
		$res1 = $db->getResult();
		  ...
		  $operate = '<a href="view_wallet.php?id='.$row['user_id'].'"><i class="fa fa-folder-open-o"></i>View Wallet</a>';
			
			$tempRow['id'] = $row['id'];
		
			$tempRow['message'] = $row['message'];
			$tempRow['amount'] = $row['amount'];
			$tempRow['name'] = $res1[0]['name'];
			
		    if($row['status']==0)
		     $tempRow['status']="<label class='label label-success'>Enable</label>";
            else
               $tempRow['status']="<label class='label label-danger'>Disable</label>";
			$tempRow['date_created'] = date('d-m-Y h:i:sa',strtotime($row['date_created']));
			$tempRow['operate'] = $operate;
