...
if(isset($_POST['type']) && $_POST['type'] != '' && $_POST['type'] == 'verify-user-email') {
	...
	if($num_rows == 0){
		//send email
		$to = $email;
		$subject = "SGrainwala Registration Verification";
		
		$message = "<#> Your OTP for SGrainwala Registration verification is : ".$otp.". Please enter this OTP to activate your profile. ";
		if($mobile){
		    sendSms($mobile, $message);   
		    $response["error"]   = false;
    		$response["message"] = "OTP is sent to your mobile number. Please verify it."/*.$smsResult*/;
